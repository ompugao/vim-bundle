...
require('lazy').setup({
  ...
  { 'saghen/blink.cmp', branch = 'main', build = 'cargo +nightly b -r' },
  ...
  { 'folke/trouble.nvim', config = function()
    require('trouble').setup({ preview = { scratch = false } })
  end },
  { 'rachartier/tiny-inline-diagnostic.nvim', config = function()
  ...
  { 'nvim-telescope/telescope.nvim', config = function()
    require('telescope').setup({
      defaults = {
        ...
        layout_config = { prompt_position = 'bottom', width = 0.99, height = 0.9 },
      },
    })
  end },
  ...
  { 'ompugao/cpsm', branch = 'feature/remove_boost_dependency', build = './install.sh' },
...
-- Open file read-only when swap exists
...
keymap('n', '<C-n>', '<Cmd>BufferNext<CR>', { silent = true })
keymap('n', '<C-p>', '<Cmd>BufferPrevious<CR>', { silent = true })
...
keymap('n', '<leader>r', '<cmd>QuickRun<CR>', { silent = true })
...
keymap('i', '<S-Tab>', function() luasnip.jump(-1) end, { silent = true })
keymap('s', '<C-k>', function() luasnip.jump(1) end, { silent = true })
keymap('s', '<Tab>', function() luasnip.jump(1) end, { silent = true })
keymap('s', '<S-Tab>', function() luasnip.jump(-1) end, { silent = true })
...
keymap('n', '<Space>rr', ':<C-u>Ref refe ')
keymap('n', '<Space>rm', ':<C-u>Ref man ')
keymap('n', '<Space>rpy', ':<C-u>Ref pydoc ')
keymap('n', '<Space>rw', ':<C-u>Ref webdict ')
...
-- quickdict.vim
keymap('n', '<Space>rq', ':<C-u>QuickDictEcho <C-r><C-w><CR>')
keymap('n', '<Space>ra', ':<C-u>QuickDictAppend <C-r><C-w><CR>')
keymap('n', '<Space>rA', ':<C-u>QuickDictInsertLast <C-r><C-w><CR>')
...
keymap('n', '<Space>gg', ':<C-u>Grepper -tool git<CR>')
keymap('n', '<Space>gi', ':<C-u>Grepper -tool rg<CR>')
keymap('n', '<Space>g*', ':<C-u>Grepper -tool rg -cword -noprompt<CR>')
...
keymap('n', '<C-l><C-p>', ':<C-u>CtrlP<CR>', { silent = true })
keymap('n', '<C-l><C-s>', ":execute ':<C-u>CtrlP ' .. expand('%:h:p')<CR>", { silent = true })
keymap('n', '<C-l><C-b>', ':<C-u>CtrlPBuffer<CR>', { silent = true })
keymap('n', '<C-l><C-r>', ':<C-u>CtrlPMRMru<CR>', { silent = true })
keymap('n', '<C-l><C-m>', ':<C-u>CtrlPMRMru<CR>', { silent = true })
keymap('n', '<C-l><CR>', ':<C-u>CtrlPMRMru<CR>', { silent = true })
keymap('n', '<C-l><C-d>', ':<C-u>CtrlPDir<CR>', { silent = true })
keymap('n', '<C-l><C-c>', ':<C-u>CtrlPQuickfix<CR>', { silent = true })
keymap('n', '<C-l><C-h>', ':<C-u>CtrlPCmdHistory<CR>', { silent = true })
keymap('n', '<C-l>/', ':<C-u>CtrlPSearchHistory<CR>', { silent = true })
keymap('n', '<C-l>l', ':<C-u>CtrlPLocate<CR>', { silent = true })
...
-- openbrowser
keymap('n', '<space>ob', '<Plug>(openbrowser-open)')
keymap('x', '<space>ob', '<Plug>(openbrowser-open)')
keymap('n', '<space>ow', '<Plug>(openbrowser-search)')
keymap('n', '<space>os', '<Plug>(openbrowser-smart-search)')
keymap('x', '<space>os', '<Plug>(openbrowser-smart-search)')

-- molder
vim.g.molder_show_hidden = 1

-- vim-easy-align
keymap('x', '<Enter>', ':LiveEasyAlign<cr>', { silent = true })

-- operator-replace
keymap('n', 'RR', 'R', { silent = true })
keymap('n', 'R', '<Plug>(operator-replace)', { silent = true })
keymap('x', 'R', '<Plug>(operator-replace)', { silent = true })

-- operator-surround
keymap({ 'n', 'x' }, 'sa', '<Plug>(operator-surround-append)', { silent = true })
keymap({ 'n', 'x' }, 'sd', '<Plug>(operator-surround-delete)', { silent = true })
keymap({ 'n', 'x' }, 'sr', '<Plug>(operator-surround-replace)', { silent = true })
...
-- asterisk
keymap('x', '*', '<Plug>(asterisk-*)')
keymap('x', 'g*', '<Plug>(asterisk-g*)')
keymap('x', '#', '<Plug>(asterisk-#)')
keymap('x', 'g#', '<Plug>(asterisk-g#)')
...
vim.api.nvim_create_autocmd('FileType', {
  pattern = 'patto',
  callback = function()
    keymap('n', '<leader>P', '<cmd>call mdip#MarkdownClipboardImage()<CR>', { buffer = true })
...
keymap('n', 'g<C-a>', 'g<Plug>(dial-increment)')
keymap('n', 'g<C-x>', 'g<Plug>(dial-decrement)')
